"""
Code which converts the keras weights to pytorch weights to suit the transformerencoder class from pytorch
"""
import sys
        ...
        for w_idx, w_transf in enumerate(w_info):
            if w_idx in Q_WEIGHTS:  # Query
                if "kernel" in w_transf.name:
                    dict_pt[pt_layer_name + "self_attn.in_proj_weight"] = np.swapaxes(w_transf.numpy(), 0, 1)
                elif "bias" in w_transf.name:
                    dict_pt[pt_layer_name + "self_attn.in_proj_bias"] = w_transf.numpy()
                else:
                    print(w_transf.name)
                    raise Exception("not kernel or bias?")
            elif w_idx in KV_WEIGHTS:  # Key and value
                if "kernel" in w_transf.name:
                    top = dict_pt[pt_layer_name + "self_attn.in_proj_weight"]
                    bot = np.swapaxes(w_transf.numpy(), 0, 1)
                    dict_pt[pt_layer_name + "self_attn.in_proj_weight"] = np.vstack((top, bot))
                elif "bias" in w_transf.name:
                    top = dict_pt[pt_layer_name + "self_attn.in_proj_bias"]
                    bot = w_transf.numpy()
                ...
                if "kernel" in w_transf.name:
                    dict_pt[pt_layer_name + "linear1.weight"] = np.swapaxes(w_transf.numpy(), 0, 1)
                elif "bias" in w_transf.name:
                ...
                if "kernel" in w_transf.name:
                    dict_pt[pt_layer_name + "linear2.weight"] = np.swapaxes(w_transf.numpy(), 0, 1)
                elif "bias" in w_transf.name:
